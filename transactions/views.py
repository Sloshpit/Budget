...
class TransactionCreate (LoginRequiredMixin, CreateView):
     ...
     #   bud_date = str(trans_date.year) +"-" +str(trans_date.month) + "-"+ "1"
        ...
        if latest_account_date.date() == trans_date_no_time:
            ...
            records_to_update = AccountBalance.objects.filter(account__account_name=acct_name, account__user=self.request.user, balance_date__gt=trans_date_no_time, balance_date__lte = now.date())
