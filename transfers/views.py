...
class CreateTransfer(CreateView):
    ...
    def form_valid(self, form):
        ...
        if now == transfer_date:
           ...
           incoming_records_to_update = AccountBalance.objects.filter(balance_date__gte=transfer_date, balance_date__lte = now, account=incoming_account)
           outgoing_records_to_update = AccountBalance.objects.filter(balance_date__gte=transfer_date, balance_date__lte = now, account=outgoing_account)           
           ...
           for record in outgoing_records_to_update:
                record.balance = record.balance - transfer_amount
                record.save()   
           latest_incoming_account = AccountBalance.objects.filter(balance_date__lt=transfer_date, account=incoming_account).order_by("-balance_date")[0]
           latest_outgoing_account = AccountBalance.objects.filter(balance_date__lt=transfer_date, account=outgoing_account).order_by("-balance_date")[0]
...
class UpdateTransfer (UpdateView):
    ...
    def form_valid(self, form):
        ...
        transfer_date = form.cleaned_data['transfer_date']
        balance_incoming_records = AccountBalance.objects.filter(account__account_name=incoming_account, balance_date__range = [transfer_date, today]  )
        ...
        balance_outgoing_records = AccountBalance.objects.filter(account__account_name=outgoing_account, balance_date__range = [transfer_date, today]  )
...
class DeleteTransfer (DeleteView):
    model = Transfer
    form_class = TransferForm
...
#    context_object_name = 'transaction'
    ...
    def delete(self, *args, **kwargs):
        ...
        incoming_account_record_to_delete = AccountBalance.objects.filter(balance_date=transfer_date, account=incoming_account).delete()
        outgoing_account_record_to_delete = AccountBalance.objects.filter(balance_date=transfer_date, account=outgoing_account).delete()

        incoming_records_to_update = AccountBalance.objects.filter(balance_date__gte=transfer_date, balance_date__lte = today, account=incoming_account)
        outgoing_records_to_update = AccountBalance.objects.filter(balance_date__gte=transfer_date, balance_date__lte = today, account=outgoing_account)
